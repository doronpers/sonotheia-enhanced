...
class CallAnalyzer:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        if model_path and Path(model_path).exists():
            try:
                self.spoof_detector.load(model_path)
                self.model_loaded = True
                logger.info(f"Loaded spoof detection model from {model_path}")
    ...
    def analyze_call(
        ...
        if codec_name not in ['landline', 'mobile', 'voip', 'clean']:
            logger.warning(f"Unknown codec {codec_name}, using landline")
            codec_name = 'landline'
...
# Global analyzer instance with trained model
model_path = Path(__file__).parent.parent / "models" / "gmm_test.pkl"
analyzer = CallAnalyzer(model_path=str(model_path) if model_path.exists() else None)
