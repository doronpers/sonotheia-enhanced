    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "metadata": {"channel": "mobile_app", "ip_address": "masked_ip_xxx"},
            }
        }
    )
...
async def update_biometric_data(
    ...
    **Returns**: Updated session information
    ...
    return SessionResponse(**session_copy)
...
async def evaluate_session_risk(
    ...
    **Returns**: Risk evaluation with decision recommendation
    ...
    # Calculate risk scores (outside lock for better performance)
    ...
    if evaluation_request.include_factors:
        factors.extend(
            [
                ...
                {"name": "Face Match", "score": biometric["face_match_score"], "weight": 0.3},
                {"name": "Liveness Check", "passed": biometric["liveness_passed"], "weight": 0.3},
                {"name": "Deepfake Detection", "score": voice["deepfake_score"], "weight": 0.4},
                {
                ...
                {"name": "Audio Quality", "score": voice["audio_quality"], "weight": 0.1},
            ]
        )
    ...
    return RiskEvaluationResponse(session_id=session_id, **risk_evaluation)
