...
class AuditEvent(BaseModel):
    ...
    score: Optional[float] = Field(None, ge=0, le=1, description="Associated score (risk, confidence, etc.)")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional event data")
    compliance_tags: List[ComplianceTag] = Field(default_factory=list, description="Compliance framework tags")
...
def mask_sensitive_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sensitive_fields = [
        "password", "pin", "ssn", "tax_id", "credit_card",
        "account_number", "routing_number", "full_address",
        "phone_number", "email", "voice_sample", "biometric_data"
    ]
...
async def generate_compliance_report(
    ...
    **Returns**: Compliance report with aggregated statistics
    """
    # Filter logs by compliance tag
    compliance_logs = [
        log for log in audit_logs_db
        if compliance_tag in log.get("compliance_tags", [])
    ]
    ...
    unique_sessions = len(set(log["session_id"] for log in compliance_logs if log.get("session_id")))
...
async def get_audit_stats(
    ...
    **Returns**: Aggregate statistics
    ...
    # Get date range
    timestamps = [log["timestamp"] for log in audit_logs_db]
    ...
    return {
        ...
        "unique_users": len(set(log["user_id"] for log in audit_logs_db if log.get("user_id"))),
        "unique_sessions": len(set(log["session_id"] for log in audit_logs_db if log.get("session_id")))
    }
...
async def purge_old_logs(
    ...
    **Returns**: Number of logs purged
    ...
    logger.warning(
        f"Audit log purge: {purged_count} logs deleted (before {before_date})"
    )
