    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "priority": "high",
                "risk_score": 0.75,
                "details": {"deepfake_score": 0.72, "face_match_score": 0.88},
            }
        }
    )
...
_escalation_counter_lock = threading.Lock()


def _get_next_escalation_id() -> str:
    """Thread-safe escalation ID generation."""
    global _escalation_counter
    with _escalation_counter_lock:
        _escalation_counter += 1
        return f"ESC-{_escalation_counter:06d}"
...
async def create_escalation(
    ...
    **Returns**: Escalation record with unique ID
    ...
    try:
        ...
        escalation_id = _get_next_escalation_id()
...
async def get_escalation(
    request: Request, escalation_id: str, api_key: Optional[str] = Depends(verify_api_key)
):
    """
    ...
    **Returns**: Complete escalation information
    ...
    with _escalations_lock:
        ...
        escalation_data = escalations_db[escalation_id].copy()
...
async def list_escalations(
    ...
    **Returns**: List of escalations
    ...
    escalations.sort(
        key=lambda e: (priority_order.get(e["priority"], 4), e["created_at"]), reverse=True
    )
...
async def assign_escalation(
    ...
    **Returns**: Updated escalation information
    ...
    with _escalations_lock:
        ...
        escalation_copy = escalation.copy()

    logger.info(f"Escalation {escalation_id} assigned to {reviewer_id}")

    return EscalationResponse(**escalation_copy)
...
@limiter.limit("100/minute")
async def get_pending_count(request: Request, api_key: Optional[str] = Depends(verify_api_key)):
    """
    ...
    **Returns**: Count of pending escalations by priority
    ...
    with _escalations_lock:
        pending_escalations = [
            e
            for e in escalations_db.values()
    ...
    counts = {
        ...
        "by_priority": {
            "critical": len(
                [e for e in pending_escalations if e["priority"] == EscalationPriority.CRITICAL]
            ),
            "high": len(
                [e for e in pending_escalations if e["priority"] == EscalationPriority.HIGH]
            ),
            "medium": len(
                [e for e in pending_escalations if e["priority"] == EscalationPriority.MEDIUM]
            ),
            "low": len([e for e in pending_escalations if e["priority"] == EscalationPriority.LOW]),
        },
        "by_status": {
            "pending": len(
                [e for e in pending_escalations if e["status"] == EscalationStatus.PENDING]
            ),
            "in_review": len(
                [e for e in pending_escalations if e["status"] == EscalationStatus.IN_REVIEW]
            ),
        },
    }
